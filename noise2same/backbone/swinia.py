...
class DiagWinAttention(nn.Module):
    ...
    def get_relative_position_bias_index_and_table(self) -> Tuple[Tensor, Tensor]:
    ...
    def head_partition_reversed(self, x: Tensor) -> Tensor:
...
class BlindSpotBlock(nn.Module):
    ...
    def strided_window_partition(self, x: Optional[Tensor]):
        if x.shape[-1] != self.embed_dim:
            return x
        expression = (
            "b (h wh sh) (w ww sw) c -> (b h w) (wh ww) (sh sw) c"
            if self.mode == SwinIAMode.SHUFFLED
            else "b (h wh sh) (w ww sw) c -> (b h w sh sw) (wh ww) c"
        )
        return einops.rearrange(
            x,
            expression,
    ...
    def strided_window_partition_reversed(
            self, x: Optional[Tensor], x_size: Iterable[int]
    ):
        height, width = x_size
        h, w = (
            height // self.window_size // self.stride,
            width // self.window_size // self.stride,
        )
        expression = (
            "(b h w) (wh ww) (sh sw) c -> b (h wh sh) (w ww sw) c"
            if self.mode == SwinIAMode.SHUFFLED
            else "(b h w sh sw) (wh ww) c -> b (h wh sh) (w ww sw) c"
        )
        return einops.rearrange(
            x, expression, h=h, w=w, sh=self.stride, sw=self.stride, wh=self.window_size
        )
    ...
    def calculate_mask(self, x_size):
        ...
        if np.any(self.shift_size != 0):
            ...
            for h, w in itertools.product(*slices):
                ...
                cnt += 1
        attn_mask = (
            self.window_partition(attn_mask)
            if self.mode == SwinIAMode.SHUFFLED
            else self.strided_window_partition(attn_mask)
        )
    ...
    def forward(
        ...
        query, key, value = map(
            self.strided_window_partition_reversed,
            (query, key, value),
            [image_size] * 3,
        )
